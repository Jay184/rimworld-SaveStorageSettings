...
namespace SaveStorageSettings {
    public class OutfitContainer : SaveableContainer {
        ...
        protected override void SaveFields(StreamWriter writer) {
            ...
            WriteField(writer, "name", Outfit.label);
                ...
                switch (key) {
                    ...
                    case "allowedHitPointsPercents":
                        if (!string.IsNullOrEmpty(value) && value.IndexOf(':') != -1) {
                            ...
                            Outfit.filter.AllowedHitPointsPercents = new FloatRange(min, max);
                            changed = true;
                        }
                        break;
                    ...
                    case "allowedQualities":
                        if (!string.IsNullOrEmpty(value) && value.IndexOf(':') != -1) {
                            string[] values = value.Split(':');
                            Outfit.filter.AllowedQualityLevels = new QualityRange(
